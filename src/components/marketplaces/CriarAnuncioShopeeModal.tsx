...
import CampoAtributo, {
  ...
  type AtributoShopee as Atributo, type ValorEscolhidoShopee as ValorEscolhido,
...
export default function CriarAnuncioShopeeModal({ canal, canais, empresaId, produtoIdInicial, origemAnuncioId, modoDuplicar, conteudoInicial, onClose, onCriado }: {
  ...
  // A regra mora junto do campo (CampoAtributoShopee), porque a tela de
  // edição precisa exatamente da mesma.
  const atributosEmJogo = atributosVisiveis(atributos, valoresAtributos)
  const preenchido = (a: Atributo) => atributoPreenchido(a, valoresAtributos)

  const atributosObrigatoriosFaltando = atributosEmJogo.filter(a => a.is_mandatory && !preenchido(a))
