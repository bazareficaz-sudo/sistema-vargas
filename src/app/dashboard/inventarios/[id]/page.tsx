...
export default async function InventarioDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: inv }, { data: itens }, { data: historico }, { data: categorias }, { data: marcas }] = await Promise.all([
    sb.from('inventarios').select('*').eq('id', id).eq('empresa_id', empresaId).single(),
    sb.from('inventario_itens').select('*').eq('inventario_id', id).order('produto_nome'),
    sb.from('inventario_historico').select('*').eq('inventario_id', id).order('created_at', { ascending: false }).limit(50),
