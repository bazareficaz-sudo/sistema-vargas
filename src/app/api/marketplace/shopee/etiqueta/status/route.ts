...
import { consultarStatusEtiqueta } from '@/lib/shopee/logistics'
...
export async function POST(req: Request) {
  ...
  try {
    const resultado = await consultarStatusEtiqueta(sb, canal, pedidoId)
    return NextResponse.json({ ok: true, ...resultado })
  } catch (e: any) {
    return NextResponse.json({ ok: false, erro: e?.message ?? 'Erro ao consultar status da etiqueta' }, { status: 400 })
  }
}
