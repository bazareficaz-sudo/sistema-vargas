...
import { getCategoryTree } from '@/lib/shopee/listing'
import type { ShopeeChannel } from '@/lib/shopee/types'
import { perfilDaSessao } from '@/lib/auth/empresaAtiva'

export async function POST(req: Request) {
  const { canalId, parentCategoryId } = await req.json()
  ...
  try {
    const categorias = await getCategoryTree({ sb, canal }, parentCategoryId ? Number(parentCategoryId) : undefined)
