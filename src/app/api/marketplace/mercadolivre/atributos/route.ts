...
import { getAtributos, getTiposAnuncio } from '@/lib/mercadolivre/listing'
...
export async function POST(req: Request) {
  ...
  try {
    const ctx = { sb, canal }
    const [atributos, tiposAnuncio] = await Promise.all([
      getAtributos(ctx, categoryId),
      getTiposAnuncio(ctx).catch(() => []),
    ])
    return NextResponse.json({ ok: true, atributos, tiposAnuncio })
  } catch (e: any) {
