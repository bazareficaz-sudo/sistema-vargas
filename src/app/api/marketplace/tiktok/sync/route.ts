...
import { syncCatalogo } from '@/lib/tiktok/sync'
import { refreshAccessTokenIfNeeded } from '@/lib/tiktok/client'
...
export async function POST(req: Request) {
  ...
  if (erroCanal || !canalRow) {
    return NextResponse.json({ ok: false, erro: 'Canal TikTok Shop não encontrado' }, { status: 404 })
  }
  ...
  let canal = montarCanal(canalRow)
  ...
  try {
    ...
    const erro = e?.message ?? 'Erro ao sincronizar com a TikTok Shop'
    await sb.from('marketplace_sync_log').insert({
