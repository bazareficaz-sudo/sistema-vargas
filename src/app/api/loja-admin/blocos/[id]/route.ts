...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!UUID.test(id)) return NextResponse.json({ erro: 'Bloco inválido' }, { status: 400 })
  ...
  const { error } = await ctx.sb
    .from('loja_blocos_home').update(resultado.valores)
    .eq('id', id).eq('loja_id', corpo.lojaId)

  if (error) {
    console.error('[loja-admin] blocos/editar falhou', { erro: error.message })
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await ctx.sb.from('loja_blocos_home').delete().eq('id', id).eq('loja_id', lojaId)
  if (error) {
    console.error('[loja-admin] blocos/excluir falhou', { erro: error.message })
